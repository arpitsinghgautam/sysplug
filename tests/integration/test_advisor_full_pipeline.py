...
def _t4_hw() -> HardwareSnapshot:
    return HardwareSnapshot(
        ...
        cpu_count=4, ram_total_mb=32_768,
    )
...
class TestMemoryFitting:
    ...
    def test_config_fits_t4_with_gpt2(self):
    ...
    def test_config_fits_on_cpu_only(self):
        adv = _advisor(hardware=_cpu_hw())
        cfg = adv.suggest_config({"batch_size": 4})
        assert cfg.batch_size >= 1
        assert cfg.learning_rate > 0

    def test_large_model_triggers_oom_recovery(self):
        ...
        tiny_gpu = HardwareSnapshot(
            ...
            cpu_count=8, ram_total_mb=32_768,
        )
    ...
    def test_gpt2_fits_comfortably_a100(self):
...
class TestTrainingTypes:
    ...
    def test_all_training_types_return_config(self, tt):
        adv = _advisor(training_type=tt)
        cfg = adv.suggest_config({"batch_size": 4})
        assert isinstance(cfg, SysPlugConfig)
        assert cfg.training_type == tt

    def test_rlhf_small_batch_adds_warning(self):
...
class TestObjectives:
    ...
    def test_all_objectives_return_config(self, obj):
...
class TestModelSpecification:
    ...
    def test_larger_model_uses_more_memory(self):
        ...
        s_cfg = small.suggest_config({"batch_size": 4, "optimizer": "adamw",
                                       "precision": "bf16"})
        l_cfg = large.suggest_config({"batch_size": 4, "optimizer": "adamw",
                                       "precision": "bf16"})
...
class TestWhatIf:
    ...
    def test_explicitly_changed_reason(self):
        adv = _advisor()
        adv.suggest_config({"batch_size": 4})
    ...
    def test_feasible_field_is_bool(self):
        adv = _advisor()
        adv.suggest_config({"batch_size": 4})
        result = adv.what_if({"batch_size": 8})
        assert isinstance(result.feasible, bool)

    def test_new_config_is_sysplug_config(self):
        adv = _advisor()
        adv.suggest_config({"batch_size": 4})
        result = adv.what_if({"batch_size": 8})
        assert isinstance(result.new_config, SysPlugConfig)

    def test_large_batch_infeasible_on_small_gpu(self):
...
class TestAdvisorMonitor:
    ...
    def test_monitor_creates_monitor_instance(self):
    ...
    def test_monitor_as_context_manager(self):
        adv = _advisor()
        adv.suggest_config({"batch_size": 4})
    ...
    def test_monitor_records_events_list(self):
        adv = _advisor()
        adv.suggest_config({"batch_size": 4})
...
class TestProfileRun:
    ...
    def test_profile_run_without_suggest_warns(self):
        import warnings
        adv = _advisor()
    ...
    def test_profile_run_with_suggest_returns_dict(self):
        adv = _advisor()
        adv.suggest_config({"batch_size": 2})
...
class TestMultiGPU:
    ...
    def test_multi_gpu_higher_effective_batch(self):
    ...
    def test_zero3_reduces_per_gpu_memory(self):
        hw = _a100_hw(count=4)
        adv_single = _advisor(hardware=_a100_hw(count=1))
        adv_zero3 = _advisor(hardware=hw)

        cfg_single = adv_single.suggest_config(
            {"batch_size": 4, "parallelism": "none"})
        cfg_zero3 = adv_zero3.suggest_config(
            {"batch_size": 4, "parallelism": "zero3"})
...
class TestConfigMethods:
    ...
    def test_to_dict_has_all_keys(self):
        ...
        cfg = adv.suggest_config({"batch_size": 4})
        d = cfg.to_dict()
        for key in ("batch_size", "learning_rate", "precision", "optimizer",
                    "parallelism", "gradient_accumulation", "effective_batch_size",
                    "use_gradient_checkpointing", "predicted_peak_memory_mb",
                    "predicted_throughput_samples_per_sec", "warnings", "notes"):
            assert key in d
    ...
    def test_repr_contains_batch(self):
        adv = _advisor()
        cfg = adv.suggest_config({"batch_size": 4})
        r = repr(cfg)
        assert "batch_size=" in r

    def test_to_deepspeed_config_bf16(self):
        adv = _advisor()
        cfg = adv.suggest_config({"batch_size": 4, "precision": "bf16"})
