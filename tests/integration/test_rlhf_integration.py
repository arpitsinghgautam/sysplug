...
class TestRLHFAdvisor:
    def test_creates_with_rlhf_training_type(
    ...
    def test_detect_reward_hacking_false_when_no_data(
        self, mock_gpu: HardwareSnapshot
    ) -> None:
        advisor = RLHFAdvisor(model="gpt2", hardware=mock_gpu, verbose=False)
        assert advisor.detect_reward_hacking() is False

    def test_detect_reward_hacking_false_when_kl_low(
        self, mock_gpu: HardwareSnapshot
    ) -> None:
    ...
    def test_detect_reward_hacking_true(
        self, mock_gpu: HardwareSnapshot
    ) -> None:
    ...
    def test_reward_window_limit(self, mock_gpu: HardwareSnapshot) -> None:
        advisor = RLHFAdvisor(
            model="gpt2", hardware=mock_gpu, verbose=False, reward_window=5
        )
        for i in range(10):
...
class TestPPOConfigHelper:
    ...
    def test_suggest_raises_without_config(
        self, mock_gpu: HardwareSnapshot
    ) -> None:
        from sysplug import Advisor
    ...
    def test_small_batch_warns(self, mock_gpu: HardwareSnapshot) -> None:
        ...
        assert any("batch" in note.lower() or "mini_batch" in note.lower()
                   for note in ppo.notes) or ppo.mini_batch_size >= 1
