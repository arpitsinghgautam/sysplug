...
class TestAdvisorWhatIf:
    ...
    def test_what_if_batch_increase_scales_lr(
        self, mock_gpu: HardwareSnapshot
    ) -> None:
    ...
    def test_what_if_infeasible_change_returns_closest(
        ...
        advisor = Advisor(
            model="llama-2-7b", hardware=mock_gpu_16gb, verbose=False
        )
    ...
    def test_what_if_multi_param_change(
        self, mock_gpu: HardwareSnapshot
    ) -> None:
    ...
    def test_what_if_diff_annotations_correct(
        self, mock_gpu: HardwareSnapshot
    ) -> None:
    ...
    def test_what_if_reason_populated(
        self, mock_gpu: HardwareSnapshot
    ) -> None:
        advisor, cfg = self._get_advisor_with_config(mock_gpu)
        result = advisor.what_if({"batch_size": 16})
    ...
    def test_what_if_without_suggest_raises(
        self, mock_gpu: HardwareSnapshot
    ) -> None:
        advisor = Advisor(model="gpt2", hardware=mock_gpu, verbose=False)
    ...
    def test_what_if_precision_change(
        self, mock_gpu: HardwareSnapshot
    ) -> None:
        advisor, cfg = self._get_advisor_with_config(mock_gpu)
    ...
    def test_what_if_feasible_field_present(
        self, mock_gpu: HardwareSnapshot
    ) -> None:
        advisor, _ = self._get_advisor_with_config(mock_gpu)
