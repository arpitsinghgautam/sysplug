...
class TestAdvisorSuggestConfig:
    ...
    def test_respects_memory_limit_16gb(self, mock_gpu_16gb: HardwareSnapshot) -> None:
    ...
    def test_suggest_with_string_model_name(self, mock_gpu: HardwareSnapshot) -> None:
    ...
    def test_current_config_set_after_suggest(self, mock_gpu: HardwareSnapshot) -> None:
        advisor = Advisor(model="gpt2", hardware=mock_gpu, verbose=False)
    ...
    def test_training_types(self, mock_gpu: HardwareSnapshot) -> None:
        for tt in ["supervised", "sft", "dpo", "rlhf", "grpo"]:
            advisor = Advisor(model="gpt2", hardware=mock_gpu, training_type=tt, verbose=False)
            cfg = advisor.suggest_config({"batch_size": 4})
