...
from sysplug.memory_model import MemoryModel, MemoryBreakdown, PrecisionMode, _params_from_name
...
class TestParameterMemoryExact:
    ...
    def test_bf16_125m(self):
        model = MemoryModel(gpu_count=1)
        est = model.predict(GPT2_PARAMS, batch_size=1, precision="bf16",
                            optimizer="sgd", parallelism="none")
        expected_params = _mb(GPT2_PARAMS, 2.0)
        assert math.isclose(est.breakdown.parameters_mb, expected_params, rel_tol=1e-6)

    def test_fp32_125m(self):
        model = MemoryModel(gpu_count=1)
        est = model.predict(GPT2_PARAMS, batch_size=1, precision="fp32",
                            optimizer="sgd", parallelism="none")
        expected_params = _mb(GPT2_PARAMS, 4.0)
        assert math.isclose(est.breakdown.parameters_mb, expected_params, rel_tol=1e-6)

    def test_int4_125m(self):
        model = MemoryModel(gpu_count=1)
        est = model.predict(GPT2_PARAMS, batch_size=1, precision="int4",
                            optimizer="sgd", parallelism="none")
        expected_params = _mb(GPT2_PARAMS, 0.5)
        assert math.isclose(est.breakdown.parameters_mb, expected_params, rel_tol=1e-6)

    def test_int8_125m(self):
        model = MemoryModel(gpu_count=1)
        est = model.predict(GPT2_PARAMS, batch_size=1, precision="int8",
                            optimizer="sgd", parallelism="none")
        expected_params = _mb(GPT2_PARAMS, 1.0)
        assert math.isclose(est.breakdown.parameters_mb, expected_params, rel_tol=1e-6)

    def test_fp16_equals_bf16(self):
        """FP16 and BF16 are both 2-byte formats — parameter memory must match."""
        model = MemoryModel(gpu_count=1)
        fp16 = model.predict(GPT2_PARAMS, batch_size=1, precision="fp16",
                              optimizer="sgd", parallelism="none")
        bf16 = model.predict(GPT2_PARAMS, batch_size=1, precision="bf16",
                              optimizer="sgd", parallelism="none")
...
class TestGradientMemoryExact:
    ...
    def test_no_parallelism_grads_equal_params(self):
        """Without ZeRO/FSDP, gradients have same dtype × size as parameters."""
        model = MemoryModel(gpu_count=1)
        est = model.predict(GPT2_PARAMS, batch_size=1, precision="bf16",
                            optimizer="sgd", parallelism="none")
        assert math.isclose(est.breakdown.gradients_mb,
                            est.breakdown.parameters_mb, rel_tol=1e-6)

    def test_zero2_shards_gradients(self):
        """ZeRO-2 divides gradient memory by gpu_count."""
        gpu_count = 4
        model = MemoryModel(gpu_count=gpu_count)
        est = model.predict(GPT2_PARAMS, batch_size=1, precision="bf16",
                            optimizer="adamw", parallelism="zero2")
        full_grad = _mb(GPT2_PARAMS, 2.0)
        assert math.isclose(est.breakdown.gradients_mb,
                            full_grad / gpu_count, rel_tol=1e-6)

    def test_zero3_shards_gradients(self):
        gpu_count = 8
        model = MemoryModel(gpu_count=gpu_count)
        est = model.predict(GPT2_PARAMS, batch_size=1, precision="bf16",
                            optimizer="adamw", parallelism="zero3")
        full_grad = _mb(GPT2_PARAMS, 2.0)
        assert math.isclose(est.breakdown.gradients_mb,
                            full_grad / gpu_count, rel_tol=1e-6)

    def test_fsdp_shards_gradients(self):
        gpu_count = 4
        model = MemoryModel(gpu_count=gpu_count)
        est = model.predict(GPT2_PARAMS, batch_size=1, precision="bf16",
                            optimizer="adamw", parallelism="fsdp")
        full_grad = _mb(GPT2_PARAMS, 2.0)
        assert math.isclose(est.breakdown.gradients_mb,
                            full_grad / gpu_count, rel_tol=1e-6)

    def test_zero1_does_not_shard_gradients(self):
        """ZeRO-1 only shards optimizer states, NOT gradients."""
        model = MemoryModel(gpu_count=4)
        est = model.predict(GPT2_PARAMS, batch_size=1, precision="bf16",
                            optimizer="adamw", parallelism="zero1")
        full_grad = _mb(GPT2_PARAMS, 2.0)
        assert math.isclose(est.breakdown.gradients_mb, full_grad, rel_tol=1e-6)

    def test_dp_does_not_shard_gradients(self):
        model = MemoryModel(gpu_count=4)
        est = model.predict(GPT2_PARAMS, batch_size=1, precision="bf16",
                            optimizer="adamw", parallelism="dp")
        full_grad = _mb(GPT2_PARAMS, 2.0)
        assert math.isclose(est.breakdown.gradients_mb, full_grad, rel_tol=1e-6)
...
class TestOptimizerStatesExact:
    ...
    def test_adamw_three_fp32_copies(self):
        """AdamW = 2 momentum tensors (fp32) + 1 master weight copy (fp32)."""
        model = MemoryModel(gpu_count=1)
        est = model.predict(GPT2_PARAMS, batch_size=1, precision="bf16",
                            optimizer="adamw", parallelism="none")
        expected = 3.0 * self._fp32_param_mb(GPT2_PARAMS)
        assert math.isclose(est.breakdown.optimizer_states_mb, expected, rel_tol=1e-6)

    def test_adam_same_as_adamw(self):
        model = MemoryModel(gpu_count=1)
        adamw = model.predict(GPT2_PARAMS, batch_size=1, precision="bf16",
                              optimizer="adamw", parallelism="none")
        adam  = model.predict(GPT2_PARAMS, batch_size=1, precision="bf16",
                              optimizer="adam",  parallelism="none")
        assert math.isclose(adamw.breakdown.optimizer_states_mb,
                            adam.breakdown.optimizer_states_mb, rel_tol=1e-6)

    def test_sgd_zero_states(self):
        model = MemoryModel(gpu_count=1)
        est = model.predict(GPT2_PARAMS, batch_size=1, precision="bf16",
                            optimizer="sgd", parallelism="none")
        assert est.breakdown.optimizer_states_mb == 0.0

    def test_adafactor_half_fp32(self):
        model = MemoryModel(gpu_count=1)
        est = model.predict(GPT2_PARAMS, batch_size=1, precision="bf16",
                            optimizer="adafactor", parallelism="none")
    ...
    def test_zero1_shards_optimizer(self):
        ...
        assert math.isclose(est.breakdown.optimizer_states_mb,
                            full_opt / gpu_count, rel_tol=1e-6)

    def test_zero3_shards_optimizer(self):
        gpu_count = 8
        model = MemoryModel(gpu_count=gpu_count)
        est = model.predict(GPT2_PARAMS, batch_size=1, precision="bf16",
                            optimizer="adamw", parallelism="zero3")
...
class TestActivationMemoryExact:
    ...
    def test_activation_scales_with_batch_size(self):
        """Doubling batch_size should double activation memory."""
        model = MemoryModel(gpu_count=1)
        est1 = model.predict(GPT2_PARAMS, batch_size=2, precision="bf16",
                             optimizer="sgd", parallelism="none",
                             hidden_dim=64, num_layers=2)
        est2 = model.predict(GPT2_PARAMS, batch_size=4, precision="bf16",
                             optimizer="sgd", parallelism="none",
                             hidden_dim=64, num_layers=2)
        ratio = est2.breakdown.activations_mb / est1.breakdown.activations_mb
        assert math.isclose(ratio, 2.0, rel_tol=1e-6)

    def test_activation_scales_with_seq_len(self):
        """Doubling sequence length should double activation memory."""
        model = MemoryModel(gpu_count=1)
        est1 = model.predict(GPT2_PARAMS, batch_size=2, precision="bf16",
                             optimizer="sgd", parallelism="none",
                             sequence_length=256, hidden_dim=64, num_layers=2)
        est2 = model.predict(GPT2_PARAMS, batch_size=2, precision="bf16",
                             optimizer="sgd", parallelism="none",
                             sequence_length=512, hidden_dim=64, num_layers=2)
        ratio = est2.breakdown.activations_mb / est1.breakdown.activations_mb
        assert math.isclose(ratio, 2.0, rel_tol=1e-6)
    ...
    def test_gradient_checkpointing_reduces_activations(self):
        ...
        model = MemoryModel(gpu_count=1)
        base = model.predict(GPT2_PARAMS, batch_size=4, precision="bf16",
                             optimizer="sgd", parallelism="none",
                             hidden_dim=64, num_layers=4)
        gc = model.predict(GPT2_PARAMS, batch_size=4, precision="bf16",
                           optimizer="sgd", parallelism="none",
                           hidden_dim=64, num_layers=4,
    ...
    def test_explicit_hidden_dim_and_layers(self):
        ...
        model = MemoryModel(gpu_count=1)
        est = model.predict(
            param_count=GPT2_PARAMS, batch_size=batch_size, precision="bf16",
            optimizer="sgd", parallelism="none", sequence_length=seq_len,
            hidden_dim=hidden_dim, num_layers=num_layers,
        )
        expected = self._expected_act_mb(
            batch_size, seq_len, hidden_dim, num_layers, bytes_elem
        )
...
class TestFrameworkOverhead:
    ...
    def test_overhead_always_500mb(self):
        model = MemoryModel(gpu_count=1)
        for prec in ("fp32", "bf16", "fp16", "int8", "int4"):
            est = model.predict(GPT2_PARAMS, batch_size=1, precision=prec,
                                optimizer="sgd", parallelism="none")
            assert est.breakdown.framework_overhead_mb == 500.0

    def test_overhead_independent_of_batch_size(self):
        model = MemoryModel(gpu_count=1)
        for bs in (1, 4, 16, 64):
            est = model.predict(GPT2_PARAMS, batch_size=bs, precision="bf16",
                                optimizer="sgd", parallelism="none")
            assert est.breakdown.framework_overhead_mb == 500.0
...
class TestTotalMemory:
    ...
    def test_total_equals_sum_of_breakdown(self):
        model = MemoryModel(gpu_count=1)
        est = model.predict(GPT2_PARAMS, batch_size=8, precision="bf16",
                            optimizer="adamw", parallelism="none")
        bd = est.breakdown
        expected_total = (
            bd.parameters_mb
    ...
    def test_peak_memory_equals_calibrated_total(self):
        """peak_memory_mb = breakdown.total_mb × calibration_factor."""
        model = MemoryModel(gpu_count=1, calibration_factor=1.25)
        est = model.predict(GPT2_PARAMS, batch_size=4, precision="bf16",
                            optimizer="adamw", parallelism="none")
        assert math.isclose(
            est.peak_memory_mb, est.breakdown.total_mb * 1.25, rel_tol=1e-9
        )
    ...
    def test_lower_bound_non_negative(self):
        ...
        for params in (1_000, 1_000_000, 7_000_000_000):
            for bs in (1, 32):
                est = model.predict(params, batch_size=bs, precision="bf16",
                                    optimizer="adamw", parallelism="none")
...
class TestZeROFullSharding:
    ...
    def test_zero3_parameters_sharded(self):
        gpu_count = 4
        model = MemoryModel(gpu_count=gpu_count)
        est = model.predict(GPT2_PARAMS, batch_size=1, precision="bf16",
                            optimizer="adamw", parallelism="zero3")
    ...
    def test_fsdp_parameters_sharded(self):
        gpu_count = 4
        model = MemoryModel(gpu_count=gpu_count)
        est = model.predict(GPT2_PARAMS, batch_size=1, precision="bf16",
                            optimizer="adamw", parallelism="fsdp")
    ...
    def test_zero2_parameters_not_sharded(self):
    ...
    def test_zero3_total_lower_than_no_parallelism(self):
...
class TestMemoryBreakdownDict:
    ...
    def test_all_keys_present(self):
        ...
        d = bd.to_dict()
        expected_keys = {
            "parameters_mb", "gradients_mb", "optimizer_states_mb",
            "activations_mb", "framework_overhead_mb", "total_mb",
        }
        assert set(d.keys()) == expected_keys

    def test_total_correct_in_dict(self):
        bd = MemoryBreakdown(
            parameters_mb=200.0, gradients_mb=200.0,
            optimizer_states_mb=600.0, activations_mb=40.0,
            framework_overhead_mb=500.0,
        )
        d = bd.to_dict()
...
class TestCalibrationExact:
    ...
    def test_perfect_fit_gives_factor_one(self):
        """If measured == predicted, calibration factor must be 1.0."""
        model = MemoryModel(gpu_count=1)
        pred  = model.predict(GPT2_PARAMS, batch_size=4, precision="bf16",
                              optimizer="adamw", parallelism="none")
        samples = [{"param_count": GPT2_PARAMS, "batch_size": 4,
                    "precision": "bf16", "optimizer": "adamw",
                    "parallelism": "none", "measured_mb": pred.peak_memory_mb}]
        factor = model.calibrate(samples)
        assert math.isclose(factor, 1.0, rel_tol=1e-6)

    def test_factor_scales_predictions(self):
    ...
    def test_factor_clamped_at_0_1(self):
        """Calibration factor is clamped to min 0.1 (sanity guard)."""
        model = MemoryModel(gpu_count=1)
        samples = [{"param_count": GPT2_PARAMS, "batch_size": 1,
                    "precision": "bf16", "optimizer": "adamw",
                    "parallelism": "none", "measured_mb": 0.001}]
        factor = model.calibrate(samples)
        assert factor >= 0.1

    def test_calibration_affects_next_prediction(self):
        """A factor of 2 must double peak_memory_mb in the next prediction."""
        model = MemoryModel(gpu_count=1)
        pred_before = model.predict(GPT2_PARAMS, batch_size=4, precision="bf16",
                                    optimizer="adamw", parallelism="none")
        samples = [{"param_count": GPT2_PARAMS, "batch_size": 4,
                    "precision": "bf16", "optimizer": "adamw",
                    "parallelism": "none",
        ...
        pred_after = model.predict(GPT2_PARAMS, batch_size=4, precision="bf16",
                                   optimizer="adamw", parallelism="none")
        assert math.isclose(pred_after.peak_memory_mb,
                            pred_before.peak_memory_mb * 2.0, rel_tol=1e-4)

    def test_multi_sample_least_squares(self):
        """With multiple samples, the least-squares solution must be returned."""
        model = MemoryModel(gpu_count=1)
        samples = []
        raw_preds = []
        for bs in (1, 2, 4, 8):
            pred = model.predict(GPT2_PARAMS, batch_size=bs, precision="bf16",
                                 optimizer="adamw", parallelism="none")
            raw_preds.append(pred.peak_memory_mb)
            # pretend measured = 1.5 × predicted
            samples.append({"param_count": GPT2_PARAMS, "batch_size": bs,
                             "precision": "bf16", "optimizer": "adamw",
                             "parallelism": "none",
...
class TestModelNameLookup:
    ...
    def test_known_models(self, name, expected_b):
    ...
    def test_predict_from_name_gpt2(self):
        model = MemoryModel(gpu_count=1)
        est = model.predict_from_name("gpt2", batch_size=4)
        assert est.peak_memory_mb > 0

    def test_predict_from_name_case_insensitive(self):
        model = MemoryModel(gpu_count=1)
...
class TestPrecisionMode:
    ...
    def test_predict_accepts_string_precision(self):
        model = MemoryModel()
    ...
    def test_predict_invalid_precision_raises(self):
        model = MemoryModel()
        with pytest.raises(ValueError):
            model.predict(GPT2_PARAMS, batch_size=1, precision="fp64",
                          optimizer="sgd", parallelism="none")
...
class TestMemoryModelProperties:
    ...
    def test_calibration_factor_default(self):
        model = MemoryModel()
        assert model.calibration_factor == 1.0

    def test_calibration_factor_constructor(self):
