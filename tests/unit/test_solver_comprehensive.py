...
class TestOutputStructure:
    ...
    def test_effective_batch_size_computed(self) -> None:
        """effective_batch_size == batch × gradient_accumulation × gpu_count."""
        solver = _solver()
        result = solver.solve(
            _base_config(batch_size=4, gradient_accumulation=4), _hw(), param_count=125_000_000
        )
    ...
    def test_predicted_throughput_positive(self) -> None:
        solver = _solver()
        result = solver.solve(_base_config(), _hw(), param_count=125_000_000)
        assert result.predicted_throughput_samples_per_sec > 0

    def test_training_type_propagated(self) -> None:
        solver = _solver(training_type="rlhf")
        result = solver.solve(_base_config(), _hw(), param_count=125_000_000)
        assert result.training_type == "rlhf"

    def test_solver_objective_propagated(self) -> None:
        solver = _solver(objective="memory")
        result = solver.solve(_base_config(), _hw(), param_count=125_000_000)
        assert result.solver_objective == "memory"

    def test_param_count_propagated(self) -> None:
        solver = _solver()
        result = solver.solve(_base_config(), _hw(), param_count=125_000_000)
        assert result.param_count == 125_000_000

    def test_sequence_length_propagated(self) -> None:
        solver = _solver()
        result = solver.solve(_base_config(), _hw(), param_count=125_000_000, sequence_length=1024)
...
class TestGradientCheckpointingRecovery:
    def test_gc_enabled_when_barely_oom(self) -> None:
    ...
    def test_gc_not_enabled_if_already_on(self) -> None:
    ...
    def test_gc_not_enabled_if_locked(self) -> None:
...
class TestBatchSizeRecovery:
    def test_batch_halved_when_oom(self) -> None:
    ...
    def test_batch_never_below_min_batch_size(self) -> None:
    ...
    def test_grad_acc_doubled_when_batch_halved(self) -> None:
        """Each batch halving doubles gradient_accumulation (preserving effective batch)."""
        solver = ConfigSolver(
            memory_model=MemoryModel(gpu_count=1),
            throughput_model=ThroughputModel(gpu_name="A100"),
            constraints=SolverConstraints(memory_safety_factor=0.85, max_grad_accumulation=64),
            training_type="supervised",
            objective="memory",
            verbose=False,
        )
    ...
    def test_batch_locked_forces_precision_downgrade(self) -> None:
...
class TestPrecisionDowngrade:
    def test_fp32_to_fp16_when_needed(self) -> None:
        solver = ConfigSolver(
            memory_model=MemoryModel(gpu_count=1),
            throughput_model=ThroughputModel(gpu_name="A100"),
            constraints=SolverConstraints(),
    ...
    def test_precision_locked_not_changed(self) -> None:
        solver = _solver()
        result = solver.solve(
...
class TestThroughputImprovement:
    def test_fp16_upgraded_to_bf16_when_budget_allows(self) -> None:
    ...
    def test_no_upgrade_when_objective_memory(self) -> None:
    ...
    def test_grad_acc_increased_for_throughput(self) -> None:
    ...
    def test_grad_acc_not_increased_if_locked(self) -> None:
        solver = _solver(objective="throughput")
        result = solver.solve(
            _base_config(gradient_accumulation=1),
            _hw(total_mb=40_960),
    ...
    def test_grad_acc_capped_by_max_constraint(self) -> None:
...
class TestLRScaling:
    def test_lr_scaled_when_effective_batch_changes(self) -> None:
        ...
        lr_original = 1e-4
        result = solver.solve(
            _base_config(batch_size=8, gradient_accumulation=1, learning_rate=lr_original),
    ...
    def test_lr_not_scaled_when_locked(self) -> None:
        lr_original = 1e-4
        solver = _solver()
        result = solver.solve(
    ...
    def test_rlhf_uses_sqrt_scaling(self) -> None:
        ...
        result_small = solver.solve(
            _base_config(batch_size=4, gradient_accumulation=1, learning_rate=1e-4),
            _hw(total_mb=2_000),
            param_count=125_000_000,
        )
    ...
    def test_linear_scaling_sft(self) -> None:
        ...
        result = solver.solve(
            _base_config(batch_size=8, gradient_accumulation=1, learning_rate=1e-4),
            _hw(total_mb=40_960),
            param_count=125_000_000,
        )
        assert result.learning_rate > 0
...
class TestStabilityWarnings:
    def test_high_lr_fp16_without_clip_warns(self) -> None:
        ...
        assert any("fp16" in w.lower() or "risky" in w.lower() for w in result.warnings)
    ...
    def test_rlhf_small_batch_warns(self) -> None:
        ...
        assert any("rlhf" in w.lower() or "reward" in w.lower() for w in result.warnings)
    ...
    def test_high_grad_acc_warns(self) -> None:
        ...
        assert any(
            "gradient_accumulation" in w or "staleness" in w.lower() for w in result.warnings
        )

    def test_no_spurious_warnings_normal_config(self) -> None:
...
class TestCPUOnlyHardware:
    def test_cpu_only_returns_valid_config(self) -> None:
        solver = _solver()
    ...
    def test_cpu_only_gpu_count_is_one(self) -> None:
        solver = _solver()
        result = solver.solve(_base_config(), _cpu_hw(), param_count=125_000_000)
...
class TestLockedParameters:
    ...
    def test_locked_param_not_changed(self, param, value) -> None:
        solver = _solver()
        result = solver.solve(
    ...
    def test_multiple_locked_params(self) -> None:
        solver = _solver()
        result = solver.solve(
...
class TestConfigNormalisation:
    def test_precision_lowercased(self) -> None:
        solver = _solver()
        result = solver.solve(
    ...
    def test_missing_gradient_accumulation_defaults_to_1(self) -> None:
        solver = _solver()
        cfg = {
            "batch_size": 8,
            "learning_rate": 1e-4,
    ...
    def test_missing_parallelism_defaults_to_none(self) -> None:
        solver = _solver()
        cfg = {
            "batch_size": 8,
            "learning_rate": 1e-4,
...
class TestHelperFunctions:
    ...
    def test_effective_batch(self, bs, acc, gpus, expected) -> None:
        assert _effective_batch(bs, acc, gpus) == expected

    @pytest.mark.parametrize(
        "pred,avail,factor,expected",
        [
    ...
    def test_is_feasible(self, pred, avail, factor, expected) -> None:
...
class TestNoIntQuantDowngrade:
    ...
    def test_never_downgrades_to_int_precision(self) -> None:
    ...
    def test_downgrades_fp32_toward_bf16_under_pressure(self) -> None:
        solver = _solver(objective="memory")
        result = solver.solve(
