...
### Added
- Initial release of SysPlug: the `Advisor` (`suggest_config`, `what_if`,
  `monitor`, `profile_run`); `SysPlugConfig` (`to_dict`,
  `to_training_arguments`, `to_deepspeed_config`, `apply_to_optimizer`,
  `summary`); `MemoryModel`; `ThroughputModel`; `ConfigSolver`;
  `StabilitySignal`; `HardwareProfiler`; and the `Monitor` background-thread
  training monitor (suggest / auto-apply / warn-only policies).
- Integrations: Hugging Face Trainer callback, DeepSpeed config patching,
  PyTorch context manager, and RLHF/PPO helpers.
- CLI: `python -m sysplug --help`, `suggest`, `hardware`, `version`.
- Examples: `raw_pytorch`, `sft_huggingface`, `dpo_custom_loop`, `rlhf_ppo`,
  `deepspeed_example`; and the paper LaTeX source (design / white paper).
...
- **Conservative, OOM-safe memory bound.** `MemoryModel` reports an asymmetric
  confidence band; the solver checks the **upper** bound for feasibility, so a
  recommended config that "fits" genuinely fits. `SysPlugConfig` gains
  `predicted_peak_memory_upper_mb`.
...
- `paper/experiments/measure_gpu.py`: a real forward+backward+optimizer harness
  ...
  GPU (with a memory-fraction cap for clean OOM on Windows/WDDM), plus
...
- CI now blocks on strict `mypy`, the full ruff ruleset, and `ruff format
  --check`, and runs a Python 3.9–3.13 matrix plus a Windows spot-check
